...
@Component({
  ...
  templateUrl: './dcsm22.component.html',
  styleUrl: './dcsm22.component.scss'
})
export class Dcsm22Component implements OnInit {
  ...
  loadData() {
    ...
    this.dcsm22Service.getAllDesignOrders(
      this.filterId,
  ...
  loadDataSorted() {
    ...
    this.dcsm22Service.getAllDesignOrdersSorted(
      this.filterId,
      this.filterjobdetails,
      this.filterowner,
      this.filterprocess,
  ...
  onRowClick(row: any) {
    if (row && row.id) {
      this.router.navigate(['/Dcsm22Detail', row.id]);
    }
  }
  ...
  fetchJobDetailFromDB(query: string) {
    this.dcsm22Service.getUniqueJobDetail(query).subscribe({
      next: (data: string[]) => {
  ...
  fetchOwnerListFromDB(query: string) {
    this.dcsm22Service.getUniqueOwner(query).subscribe({
      next: (data: string[]) => {
        this.OwnerList = data;
      },
      error: (err) => { }
  ...
  fetchAssigneeFromDB(query: string) {
    this.dcsm22Service.getUniqueAssignee(query).subscribe({
      next: (data: string[]) => {
        this.Assignee = data;
      },
      error: (err) => { }
  ...
  fetchProcessFromDB(query: string) {
    this.dcsm22Service.getUniqueProcess(query).subscribe({
      next: (data: string[]) => {
        this.Process = data;
      },
      error: (err) => { }
  ...
  fetchConfirmFromDB(query: string) {
    this.dcsm22Service.getUniqueConfirm(query).subscribe({
      next: (data: string[]) => {
        this.Confirm = data;
      },
      error: (err) => { }
  ...
  Backlog() {
    this.dcsm22Service.countBacklog().subscribe({
      next: (data: number) => {
  ...
  countBacklogInProgress() {
    this.dcsm22Service.countBacklogInProgress().subscribe({
      next: (data: number) => {
  ...
  countBacklogPending() {
    this.dcsm22Service.countBacklogPending().subscribe({
      next: (data: number) => {
        this.pending = data;
      },
      error: (err) => { }
  ...
  countBacklogCheck() {
    this.dcsm22Service.countBacklogCheck().subscribe({
      next: (data: number) => {
        this.checked = data;
      },
      error: (err) => { }
  ...
  countBacklogEdit() {
    this.dcsm22Service.countBacklogEdit().subscribe({
      next: (data: number) => {
        this.edited = data;
      },
      error: (err) => { }
  ...
  countBacklogComplete() {
    this.dcsm22Service.countBacklogComplete().subscribe({
      next: (data: number) => {
        this.completed = data;
      },
      error: (err) => { }
