...
@Component({
  ...
  templateUrl: './dcsm21-detail.component.html',
  styleUrl: './dcsm21-detail.component.scss'
})
export class Dcsm21DetailComponent implements OnInit {
  ...
  constructor(
    ...
    private dcsm21Service: Dcsm21Service,
  ...
  onSubmit(): void {
    if (this.designForm.valid) {
      ...
      this.dcsm21Service.save(data).subscribe((response) => {
        try {
          this.patchFormData(response);
          this.loadingService.hide();
          this.checkBtn();
          this.sweetAlert.success('Success', 'บันทึกข้อมูลสำเร็จ!');
          this.router.navigate(['/Dcsm21']);
        } catch (error) {
          this.loadingService.hide();
  ...
  updateStatusEdit() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.loadingService.show();
        this.dcsm21Service.updateStatusEdit(this.designForm.getRawValue().id).subscribe((response) => {
          this.designForm.patchValue(response);
          this.loadingService.hide();
          this.checkBtn();
