...
import { Dcsm21Service } from './dcsm21.service';
...
@Component({
  ...
  templateUrl: './dcsm21.component.html',
  styleUrl: './dcsm21.component.scss'
})
export class Dcsm21Component implements OnInit {
  ...
  constructor(private http: HttpClient, private dcsm21Service: Dcsm21Service, private router: Router, private statusColorService: StatusColorService) { }
  tableColumns = [
    { key: 'id', label: 'ลำดับ' },
  ...
  loadData() {
    ...
    this.dcsm21Service.getAllDesignOrders(
      this.filterId,
      this.filterfolder,
  ...
  loadDataSort() {
    ...
    this.dcsm21Service.getAllDesignOrdersSort(
      this.filterId,
      this.filterfolder,
      this.filterjobdetails,
      this.filterowner,
  ...
  onRowClick(row: any) {
    if (row && row.id) {
      this.router.navigate(['/Dcsm21Detail', row.id]);
    }
  }

  add() {
    this.router.navigate(['/Dcsm21Detail']);
  }
  ...
  countBacklogCheck() {
    this.dcsm21Service.countBacklogCheck().subscribe({
      next: (data: number) => {
        this.checked = data;
      },
      error: (err) => { }
