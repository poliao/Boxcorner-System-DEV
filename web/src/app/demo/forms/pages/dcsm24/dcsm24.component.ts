...
import { Dcsm24Service } from './dcsm24.service';
...
@Component({
  ...
  templateUrl: './dcsm24.component.html',
  styleUrl: './dcsm24.component.scss'
})
export class Dcsm24Component implements OnInit {
  ...
  constructor(
    ...
    private dcsm24Service: Dcsm24Service,
  ...
  loadData() {
    ...
    this.dcsm24Service.getAllDesignOrders(
      this.filterId,
  ...
  loadDataSorted() {
    ...
    this.dcsm24Service.getAllDesignOrdersSorted(
      this.filterId,
      this.filterjobdetails,
      this.filterowner,
      this.filterprocess,
  ...
  onRowClick(row: any) {
    if (row && row.id) {
      this.router.navigate(['/Dcsm24Detail', row.id]);
    }
  }
  ...
  fetchJobDetailFromDB(query: string) {
    this.dcsm24Service.getUniqueJobDetail(query).subscribe({
      next: (data: string[]) => {
  ...
  fetchOwnerListFromDB(query: string) {
    this.dcsm24Service.getUniqueOwner(query).subscribe({
      next: (data: string[]) => {
        this.OwnerList = data;
      },
      error: (err) => { }
  ...
  fetchAssigneeFromDB(query: string) {
    this.dcsm24Service.getUniqueAssignee(query).subscribe({
      next: (data: string[]) => {
        this.Assignee = data;
      },
      error: (err) => { }
  ...
  fetchProcessFromDB(query: string) {
    this.dcsm24Service.getUniqueProcess(query).subscribe({
      next: (data: string[]) => {
        this.Process = data;
      },
      error: (err) => { }
  ...
  fetchConfirmFromDB(query: string) {
    this.dcsm24Service.getUniqueConfirm(query).subscribe({
      next: (data: string[]) => {
        this.Confirm = data;
      },
      error: (err) => { }
  ...
  Backlog() {
    this.dcsm24Service.countBacklog().subscribe({
      next: (data: number) => {
  ...
  countBacklogInProgress() {
    this.dcsm24Service.countBacklogInProgress().subscribe({
      next: (data: number) => {
  ...
  countBacklogPending() {
    this.dcsm24Service.countBacklogPending().subscribe({
      next: (data: number) => {
        this.pending = data;
      },
      error: (err) => { }
  ...
  countBacklogCheck() {
    this.dcsm24Service.countBacklogCheck().subscribe({
      next: (data: number) => {
        this.checked = data;
      },
      error: (err) => { }
  ...
  countBacklogEdit() {
    this.dcsm24Service.countBacklogEdit().subscribe({
      next: (data: number) => {
        this.edited = data;
      },
      error: (err) => { }
  ...
  countBacklogComplete() {
    this.dcsm24Service.countBacklogComplete().subscribe({
      next: (data: number) => {
        this.completed = data;
      },
      error: (err) => { }
