...
@Component({
  selector: 'app-dcsm24-detail.component',
  imports: [ReactiveFormsModule, CommonModule, MatIconModule,],
  templateUrl: './dcsm24-detail.component.html',
  styleUrl: './dcsm24-detail.component.scss'
})
export class Dcsm24DetailComponent implements OnInit {
  ...
  updateStatus() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.dcsm24Service.updateStatus(this.designForm.getRawValue().id).subscribe((response) => {
          this.designForm.patchValue(response);
          this.checkButtonVisibility();
          this.loadingService.hide();
          this.sweetAlert.success('Success', 'ยอมรับงานสำเร็จ!');
          this.router.navigate(['/Dcsm24']);
        })
      }
    });
  }
  ...
  updateStatusWorking() {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.dcsm24Service.save(this.designForm.getRawValue()).subscribe((response) => {
          this.designForm.patchValue(response);
          this.checkButtonVisibility();
          this.loadingService.hide();
