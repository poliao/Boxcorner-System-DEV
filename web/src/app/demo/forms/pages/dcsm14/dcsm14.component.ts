...
@Component({
  ...
  templateUrl: './dcsm14.component.html',
  styleUrls: ['./dcsm14.component.scss']
})
export class Dcsm14Component implements OnInit {
  ...
  constructor(private dcsm14Service: Dcsm14Service, private router: Router) { }
  ...
  loadData() {
    this.dcsm14Service.getAll(this.pageIndex, this.pageSize, {
      id: this.filterId,
  ...
  add() {
    this.router.navigate(['/Dcsm14Detail']);
  }

  onRowClick(row: any) {
    if (row && row.id) {
      this.router.navigate(['/Dcsm14Detail', row.id]);
    }
  }
}
