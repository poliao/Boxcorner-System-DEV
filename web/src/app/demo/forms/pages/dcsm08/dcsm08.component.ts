...
@Component({
  ...
  templateUrl: './dcsm08.component.html',
  styleUrls: ['./dcsm08.component.scss']
})
export class Dcsm08Component implements OnInit {
  ...
  loadData(): void {
    ...
    this.dcsm08Service.getOrdersWithSearch(apiFilters).subscribe({
      next: (res: any) => {
  ...
  loadDataSort(): void {
    ...
    this.dcsm08Service.getOrdersWithSearchSort(apiFilters).subscribe({
      next: (res: any) => {
        this.tableData = res.content.map((item: any) => ({
          ...item,
          deadlineDate: this.formatDate(item.deadlineDate),
