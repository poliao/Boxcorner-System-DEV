...
@Component({
  ...
  templateUrl: './dcsm23.component.html',
  styleUrl: './dcsm23.component.scss'
})
export class Dcsm23Component implements OnInit {
  ...
  constructor(private http: HttpClient, private dcsm23Service: Dcsm23Service, private router: Router, private statusColorService: StatusColorService) { }
  tableColumns = [
    { key: 'id', label: 'ลำดับ' },
  ...
  loadData() {
    ...
    this.dcsm23Service.getAllDesignOrders(
      this.filterId,
      this.filterfolder,
  ...
  loadDataSort() {
    ...
    this.dcsm23Service.getAllDesignOrdersSort(
      this.filterId,
      this.filterfolder,
      this.filterjobdetails,
      this.filterowner,
  ...
  onRowClick(row: any) {
    if (row && row.id) {
      this.router.navigate(['/Dcsm23Detail', row.id]);
    }
  }

  add() {
    this.router.navigate(['/Dcsm23Detail']);
  }
  ...
  countBacklogCheck() {
    this.dcsm23Service.countBacklogCheck().subscribe({
      next: (data: number) => {
        this.checked = data;
      },
      error: (err) => { }
