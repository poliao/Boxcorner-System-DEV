...
export class Dcsm23Service {
  ...
  save(data: any): Observable<any> {
    // ปรับ Path ให้ตรงกับ Backend ของคุณ (ตัวอย่างเช่น /designs-diecut/save)
  ...
  updateStatusApprove(id: number): Observable<any> {
    return this.http.put(`${this.apiUrl}/designs-diecut/updateStatusApprove?id=${id}`, {});
  }

  updateStatusEdit(id: number): Observable<any> {
    return this.http.put(`${this.apiUrl}/designs-diecut/updateStatusEdit?id=${id}`, {});
  }

  countBacklogCheck(): Observable<any> {
    return this.http.get(`${this.apiUrl}/designs-diecut/countBacklogCheck`);
  }
}
